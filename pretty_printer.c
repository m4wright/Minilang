...
void pretty_print_statements(STATEMENTS *statements);
...
void pretty_print_while(STATEMENT *statement){
	fprintf(output_pretty, "while ");
	pretty_print_expression(statement->val.WHILE.condition);
	fprintf(output_pretty, " do\n");
	pretty_print_statements(statement->val.WHILE.statements);
	fprintf(output_pretty, "done\n");
}


void pretty_print_else(STATEMENTS *statements){
...
void pretty_print_if(STATEMENT *statement){
	fprintf(output_pretty, "if ");
	pretty_print_expression(statement->val.IF.condition);
	fprintf(output_pretty, " then\n");
	pretty_print_statements(statement->val.IF.statements);
	pretty_print_else(statement->val.IF.else_stmts);
	fprintf(output_pretty, "endif\n");
}

void pretty_print_print_stmt(STATEMENT *statement){
	fprintf(output_pretty, "print ");
	pretty_print_expression(statement->val.PRINT.to_print);
	fprintf(output_pretty, ";\n");
}

void pretty_print_read_stmt(STATEMENT *statement){
	fprintf(output_pretty, "read %s;\n", statement->val.to_read.identifier);
}

void pretty_print_assignment(STATEMENT *statement){
...
void pretty_print_statement(STATEMENT *statement){
	switch (statement->statement_type){
		case WHILE_STMT:
			pretty_print_while(statement);
			break;
		case IF_STMT:
			pretty_print_if(statement);
			break;
		case PRINT_STMT:
			pretty_print_print_stmt(statement);
			break;
		case READ_STMT:
			pretty_print_read_stmt(statement);
			break;
		case ASSIGNMENT_STMT:
			pretty_print_assignment(statement);
			break;
		default:
...
void pretty_print_statements(STATEMENTS *statements){
	while (statements){
		pretty_print_statement(statements->statement);
